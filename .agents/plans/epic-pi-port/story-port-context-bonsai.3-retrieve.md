...
**No same-step guard.** Pi intentionally supports same-turn prune+retrieve as a successful audit-clean no-op. The session records both archive and archive-clear entries, and tombstone precedence ensures the range is restored immediately.
...
- Model that prunes and then immediately realises it needs the content — same-turn retrieve should just work.
...
### Design Implications
- Tombstone semantics are cleaner than "delete the archive custom entry" because the session file is append-only; archive and retrieval entries stay on record for audit when retrieval is allowed.
...
- [ ] If required primitives are unavailable or incompatible (`pi.appendEntry` or session/archive state), retrieve fails closed with a deterministic plain-text compatibility error and does not mutate archive state.
...
- Integration test covering same-turn prune+retrieve (one assistant message, both tools succeed, next context is un-elided).
...
- **Unit**: same-turn no-op path, tombstone write path.
