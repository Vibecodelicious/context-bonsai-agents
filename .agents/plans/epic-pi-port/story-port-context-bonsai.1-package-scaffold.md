...
### User Gamut
- Pi maintainer reviewing a fresh standalone extension package for consistency with how extensions are normally authored.
...
- [ ] `pi_context_bonsai/package.json` exists with:
  ...
  - Runtime dep on `@mariozechner/pi-coding-agent` (explicit pinned published version) for the extension types; dev dep on `vitest` for unit tests.
...
- [ ] `pi_context_bonsai/tsconfig.json` extends `../../tsconfig.base.json`. No `outDir` required for this story; add `"noEmit": true` and `"include": ["src/**/*", "test/**/*"]` so `tsgo --noEmit` at the repo root still type-checks it.
- [ ] `pi_context_bonsai/src/index.ts` exports a default `ExtensionFactory` that:
...
- [ ] `pi_context_bonsai/src/prompt.ts` exports `BONSAI_GUIDANCE` — port verbatim from `/home/basil/projects/opencode_context_bonsai_plugin/src/prompt.ts` `getSystemPromptGuidance()`.
- [ ] One unit test in `pi_context_bonsai/test/` that imports the factory and asserts handler registration for `before_agent_start` (no runtime invocation needed).
- [ ] One integration test in `pi_context_bonsai/test/integration/01-scaffold.test.ts` that:
...
- [ ] `npm --prefix pi_context_bonsai test` passes.
...
### New Files to Create
- `pi_context_bonsai/package.json`
- `pi_context_bonsai/tsconfig.json`
- `pi_context_bonsai/src/index.ts`
- `pi_context_bonsai/src/prompt.ts`
- `pi_context_bonsai/test/prompt.test.ts`
- `pi_context_bonsai/test/integration/01-scaffold.test.ts`
...
- Scaffold `pi_context_bonsai/` with just `package.json` and `tsconfig.json` (no build config). Use the script shape from `packages/coding-agent/package.json` for `"test": "vitest --run"`, and the `"clean"/"check"` stubs from `packages/coding-agent/examples/extensions/with-deps/package.json` for consistency with the in-repo extension example.
- Add `@mariozechner/pi-coding-agent` as a dependency at an explicit pinned published version, alongside `@mariozechner/pi-agent-core`; the side repo is a plain standalone npm package, not a monorepo workspace member.
...
- Integration test under `pi_context_bonsai/test/integration/01-scaffold.test.ts`:
  - Import the factory directly from the side repo's `src/`.
  - Run one prompt through the faux provider, assert the `BuildSystemPromptOptions`'s effective prompt contains `BONSAI_GUIDANCE`.

### Phase 4: Gates
- Run `npm run typecheck` and `npm test` in the side repo. Fix all errors/warnings.
...
3. Create `pi_context_bonsai/package.json`, `tsconfig.json`, (and `tsconfig.build.json` if the mom package uses that pattern).
4. Run `npm install` in the side repo to materialise its dependencies.
5. Create `pi_context_bonsai/src/prompt.ts` with the verbatim-port guidance.
6. Create `pi_context_bonsai/src/index.ts` with the factory described above.
7. Create `pi_context_bonsai/test/prompt.test.ts` (unit).
8. Create `pi_context_bonsai/test/integration/01-scaffold.test.ts` (integration).
...
- `cd /home/basil/projects/context-bonsai-agents/pi_context_bonsai && npx tsx ../../node_modules/vitest/dist/cli.js --run test/prompt.test.ts`
- `cd /home/basil/projects/context-bonsai-agents/pi_context_bonsai && npx vitest run test/suite/context-bonsai/01-scaffold.test.ts`

## Worktree Artifact Check

- Checked At: 2026-04-23
- Planned Target Files: `pi_context_bonsai/package.json`, `pi_context_bonsai/tsconfig.json`, `pi_context_bonsai/src/index.ts`, `pi_context_bonsai/src/prompt.ts`, `pi_context_bonsai/test/prompt.test.ts`, `pi_context_bonsai/test/integration/01-scaffold.test.ts`
