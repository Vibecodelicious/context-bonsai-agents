...
| `structural-unbound-harness` | The target harness has no bound slot table in Part 4 (§"How a routine cycle uses this spec", §4.7) | Nothing exists to invalidate — level-2 is absent | Owner-tier new-harness derivation |
...
Each supported harness binds the slots below before the routine path can run against it. OpenCode, Claude Code, and Pi are bound from executed, proven cycles; Codex and Cline are bound by the derivation pipeline's Stage 6 (`derivation-pipeline-spec.md` §9) and await their first executed cycles. **Do not improvise values for unbound harnesses** — binding a new harness is derivation work (the new-harness path in `../meta-loop-direction.md`), not a routine cycle.
...
- **Baseline rows** (clean upstream at target):
  - r01 (must-be-green), from the worktree's `codex-rs/`: `cargo build --bin codex`. Memory-constrained rigs use the documented accommodation `CARGO_INCREMENTAL=0 CARGO_PROFILE_DEV_DEBUG=0 cargo build --bin codex` (`codex_context_bonsai/docs/e2e-results-2026-05-20-live.md`, Setup).
...
- **E2E procedure and required scenarios**: procedure `cline_context_bonsai/docs/e2e-testing.md` (combined installation+runtime document; binds all eight e2e-spec per-harness slots). **Command binding**: `docs/cline-e2e-runbook.md` — cycle plans copy commands from it; if it is absent or does not name a command the plan needs, generation must flag the missing binding instead of inventing commands (the §4.2 rule). Required minimum for a release-gate PASS (immutable per cycle, §2.9): live tool registration, E2E-01 (contiguous prune), E2E-02 (ambiguity rejection), E2E-03 (retrieve by anchor), E2E-06 CLI half (persistence across resume), and E2E-07/Protocol A (secret-prune oracle) — all EXECUTED 2026-07-06 against the derivation-time build — **plus the port's open live scenario, E2E-04 (gauge): bound but never executed live; the first routine cycle that reaches the live e2e gate must execute it, and no full parity claim exists until it passes together with a fresh-machine installation run** (Stage 5 record in `cline_context_bonsai/docs/e2e-testing.md`). E2E-05 is justified non-live there (the history-overwrite seam is compiled into the same bundle as the tools and not independently degradable); its fail-closed behavior is carried by the side-repo `npm test` row and the fork's `ContextBonsaiApplier` tests, and the justification expires if a future release makes the seam degradable. E2E-06's VS Code checkpoint-restore half is the recorded manual half — bound, driven manually, never auto-claimed.
...
## 4.7 Unbound harnesses

`gemini-cli` and `kilo` have Context Bonsai ports but no executed forward-port cycle and no bound slot table. Running the routine path against them requires first emitting their bindings via the derivation pipeline (`derivation-pipeline-spec.md`). Their per-harness spec/bindings pairs live in this directory; those documents do not substitute for a slot table.
